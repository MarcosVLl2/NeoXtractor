...
class PointRenderer:
    ...
    def __init__(
        self,
        rhi_widget: QtWidgets.QRhiWidget,
    ...
    def _points_to_quads(
        self, points: list[tuple[int, int, int, int]], screen_width, screen_height
    ):
        """
    ...
    def initialize(self, cb: QtGui.QRhiCommandBuffer, mvp_ubuf: QtGui.QRhiBuffer):
        ...
        if self._shaders is None:
            ...
            with open(os.path.join(shaders_path, f"{shader_name}.vert.qsb"), "rb") as f:
                vsrc = f.read()
                with open(
                    os.path.join(shaders_path, f"{shader_name}.frag.qsb"), "rb"
                ) as f:
                    fsrc = f.read()
                    self._shaders = (
        ...
        if self._pipeline is None:
            if self._is_d3d:
                self._vert_ubuf = self._rhi.newBuffer(
            ...
            self._frag_ubuf = self._rhi.newBuffer(
            ...
            bindings = [
                QtGui.QRhiShaderResourceBinding.uniformBuffer(
                    0, QtGui.QRhiShaderResourceBinding.StageFlag.VertexStage, mvp_ubuf
                ),
                QtGui.QRhiShaderResourceBinding.uniformBuffer(
                    1,
            ...
            self._pipeline.setShaderStages(
                [
                    QtGui.QRhiShaderStage(
                        QtGui.QRhiShaderStage.Type.Vertex, self._shaders[0]
                    ),
                    QtGui.QRhiShaderStage(
                        QtGui.QRhiShaderStage.Type.Fragment, self._shaders[1]
                    ),
                ]
            )
            ...
            input_layout.setBindings(
                [
                    QtGui.QRhiVertexInputBinding(
                        (6 if self._is_d3d else 4) * ctypes.sizeof(ctypes.c_float)
                    ),
                ]
            )
            input_attributes = [
                ...
                QtGui.QRhiVertexInputAttribute(
                    0,
                    1,
                    QtGui.QRhiVertexInputAttribute.Format.Float,
                    3 * ctypes.sizeof(ctypes.c_float),
                ),
            ]
            if self._is_d3d:
        ...
        if self._is_d3d:
            if self._vert_ubuf is not None:
                ...
                resource_updates.updateDynamicBuffer(
                    self._vert_ubuf, 0, ctypes.sizeof(arr), cast(int, arr)
                )

            if self._frag_ubuf is not None:
                arr = (ctypes.c_float * len(self._point_color))(*self._point_color)
                resource_updates.updateDynamicBuffer(
                    self._frag_ubuf, 0, ctypes.sizeof(arr), cast(int, arr)
                )
        ...
        if self._new_points:
            ...
            if self._is_d3d:
                ...
                resource_updates.uploadStaticBuffer(
                    cast(QtGui.QRhiBuffer, self._ibuf), cast(int, arr)
                )
