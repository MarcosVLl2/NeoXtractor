...
class MeshParser5(BaseMeshParser):
    ...
    def _parse_mesh_enhanced(
        self, model: dict[str, Any], f: BinaryIO
    ) -> dict[str, Any]:
        ...
        if model["has_bones"] != 0 and model["has_bones"] != 3:
            ...
            if _flag != 0:
                raise ValueError(
                    f"Unexpected _flag value {_flag} at position {f.tell()}"
                )
        ...
        model["mesh"]["uv"] = []
        # vertex uv
        for mesh_vertex_count, _, uv_layers, color_len in model["mesh"]["data"]:
            if uv_layers > 0:
